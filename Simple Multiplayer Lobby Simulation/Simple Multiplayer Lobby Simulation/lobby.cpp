...
void Lobby::AssignStats() {
    ...
    for (int i = 0; i < maxSize; ++i) {
        kd = static_cast<float>(rand() % 201 + 50) / 100.0;
        wl = static_cast<float>(rand() % 201 + 50) / 100.0;
        playersInLobby.emplace_back("Player" + std::to_string(i + 1), kd, wl);
    }
}
